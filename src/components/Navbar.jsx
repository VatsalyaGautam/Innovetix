...
const Navbar = () => {
  ...
  return (
    ...
    <>
      ...
      {/* Navbar */}
      <nav
        className={`navbar fixed top-0 left-0 right-0 z-50 w-full bg-black border-b border-gray-800 transition-all duration-300 animate-slideInTop  ${
            ...
            <div className="hidden md:flex items-center justify-center gap-2 lg:gap-7">
              {navigationItems.map((item, index) => (
                <div key={item.title} className="relative group">
                  <Link
                    href={item.link}
                    className={`relative p-1 text-sm lg:text-base font-medium text-white hover:bg-slate-900 hover:scale-105 transition-transform duration-300 ease-in-out rounded-md lg:p-3 ${
                        ...
                        <div className="relative bg-white rounded-lg p-4 grid grid-cols-2 gap-4">
                          {item.dropdownItems.map((dropdownItem, idx) => (
                            <Link
                              ...
                              key={idx}
                              className="flex items-start space-x-4 p-3 rounded-lg hover:bg-gray-100 transition-colors"
                            >
          ...
          <div className="flex flex-col space-y-4 p-4">
            {navigationItems.map((item, index) => (
              <Link
                key={item.title}
                href={item.link}
                className="text-white hover:bg-slate-900 rounded-md py-2 px-3"
              >
                {item.title}
              </Link>
            ))}
